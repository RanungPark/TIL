...
link등을 표기할때 그길이가 매우 길어지는데 이 프로퍼티를 사용하지 않으면 부모영역을 넘어가게 된다<br>
`wovd-wrap : break-word;`
## word-break 프로퍼티
한단어의 길이가 길어서 부모 영역을 벗어난 텍스트 처리 방법을 정의한다<br>
word-wrap 프로퍼티는 단어를 어느 정도 고려했지만 word-break는 단어를 고려하지 않고 부모 영역에 맞추어 강제 개행한다<br>
`wovd-break : break-all;`
...
# 요소정렬
- float 프로퍼티는 주로 레이아웃을 구성할 때 블록 레벨 요소를 가로 정렬하기 위해 사용되어 중요한 기법이다
- 본래 이미지와 텍스트가 있을때 이미지 주위를 텍스트로 감싸기 위해 만들어졌다
- none : 요소를 떠 있게 하지 않는다
- right : 요소를 오른쪽으로 이동시킨다
- left : 요소를 왼쪽으로 이동시킨다
## 정렬
float 프로퍼티를 사용하지 않은 블록 요소들은 수직으로 정렬된다
  `float:left;`프로퍼티를 사용하면 왼쪽부터 가로 정렬되고 `float:right;` 프로퍼티는 오른쪽 부터 가로 정렬된다<br>
오른쪽 가로 정렬의 경우 먼저 기술된 요소가 가장 오른쪽에 출력되므로 출력 순서가 역순이 된다

#### 중앙 가로 정렬은 margin 프로퍼티를 사용해야 한다 `margin : 0 auto;`

## width
- width 프로퍼티의 기본값은 100%이므로 지정하지 않은 block요소는 부모 요소의 가로폭을 가득 채운다
- width 프로퍼티를 선언하지 않은 block 레벨 요소에 float 프로퍼티가 선언되면 width가 inline 요소와 같이 content에 맞게 최소화되고 다음 요소 위에 떠있게 된다

## float프로퍼티 관련 문제 해결 방법
### float 프로퍼티가 선언된 요소와 float 프로퍼티가 선언되지 않은 요소간 margin이 사라지는 문제
- overflow : hidden을 선언한다
- float를 선언하면 너비가 최소화가 된다
### float 프로퍼티가 선언된 자식 요소를 포함하는 부모 요소의 높이가 정상적으로 반영되지 않는 문제
- overflow : hidden을 선언한다
- container 영역이 끝나기 직전 빈요소를 만들고 clear:both를 선언하는 방법도 가능하다
- ::after 가상 요소 선택자를 이용한다
  `.clearfix:after {content: ""; display: block; clear: both;}`
- flat 프로퍼티 대신 `dsplay : inline-block;` 을 선언한다 
  이때 부모요소에 `font-size : 0;`을 선언하여 두 요소 사이에 정의하지 않은 공백을 제거한다
